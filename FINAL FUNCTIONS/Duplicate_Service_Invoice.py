...
# Example usage
csv_path = r"C:\Users\gyana\OneDrive\Desktop\MMC CONVERT\Myob To Xero Python\Data Sets\SAMPLE DATA SETS\SI-FERRAR.xlsx"
...
duplicate_numbers_rows = df[df['Number'].isin(duplicate_numbers)]

# Step 3: Find Numbers with multiple unique OriginalIDs
id_uniqueness = duplicate_numbers_rows.groupby('Number')['OriginalID'].nunique()
...
def create_invoice_number(row):
    if row['Number'] in numbers_with_multiple_ids:
        return f"{row['Number']}-{row['OriginalID'][:8]}"
    else:
        return row['Number']
...
df.to_csv(r"C:\Users\gyana\OneDrive\Desktop\XERO_Duplicate_Service_Invoice.csv", index=False, encoding='utf-8-sig')
