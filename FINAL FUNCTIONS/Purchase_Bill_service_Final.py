...
# Load data files
csv_path = r"C:\Users\gyana\OneDrive\Desktop\MMC CONVERT\Myob To Xero Python\Data Sets\Infracorr Consulting Pty Ltd (Neharika)\MYOB-RAW-DATA\Service Bills.csv"
...
print(df['*TaxType'].head())

def map_tax_code(row):
    if "*AccountCode" not in row or "*TaxType" not in row:
        return "BAS Excluded"

    if pd.isna(row["*AccountCode"]) or row["*AccountCode"] == "":
        account_code = ""
    else:
        try:
            account_code = str(int(float(row["*AccountCode"])))
        except:
...
columns_order = [
    ...
    "TrackingName1", "TrackingOption1", "TrackingName2", "TrackingOption2", "Currency", "Exchange Rate"
]
...
# Now export with all columns guaranteed to be present
df[columns_order].to_csv(
    r"C:\Users\gyana\OneDrive\Desktop\MMC CONVERT\Myob To Xero Python\Data Sets\Infracorr Consulting Pty Ltd (Neharika)\XERO DATA\XERO_PURCHASE_BILL_SERVICE_2.csv",
    index=False,
    encoding="utf-8"
)
