...
# csv_path = r"C:\Users\gyana\OneDrive\Desktop\MMC CONVERT\Myob To Xero Python\Data Sets\Infracorr Consulting Pty Ltd (Neharika)\MYOB-RAW-DATA\SPENDMONEY.csv"
...
# # Optional: Adjust column width if they are too long
...
# # print("Valid Account Codes (Bank/Credit Card):", valid_codes)
...
#         return matching_row["Job Number"].fillna('').values[0] + "-" + matching_row["Job Name"].fillna('').values[0]
#     else:
#         return ""
...
# import numpy as np
# df['Bank'] = df['Bank'].replace(r'^\s*$', np.nan, regex=True)
# df['Bank'] = df['Bank'].ffill()
...
# df_bank['Bank'] = df_bank['Bank'].ffill()
...
# # print("Sample df['Account Code'] values:", df['Account Code'].dropna().unique()[:10])
...
# # Replace empty strings with NaN, then convert to float safely
# print(df['Amount'].head())
# df_bank["Amount"] = df_bank["Amount"].astype(str).str.replace(",", "", regex=False).str.replace(r"\((.*?)\)", r"-\1", regex=True).replace('', np.nan).astype(float)
# df["Amount"] = df["Amount"].astype(str).str.replace(",", "", regex=False).str.replace(r"\((.*?)\)", r"-\1", regex=True).replace('', np.nan).astype(float)
# df["Tax Amount"] = df["Tax Amount"].astype(str).str.replace(",", "", regex=False).str.replace(r"\((.*?)\)", r"-\1", regex=True).replace('', np.nan).astype(float)
...
# df["Bank"] = df["Bank"].astype(str).str.strip().replace('', np.nan)
# df['Bank'] = df['Bank'].ffill()
...
# df = df[df['Account Code'].notna() & (df['Account Code'].str.strip() != '')].reset_index(drop=True)
# df['Account Code'] = pd.to_numeric(df['Account Code'], errors='coerce')
# valid_refs = df[df['Account Code'].notna()]['Reference'].unique()
# df = df[(df['Reference'].isin(valid_refs)) & (df['Account Code'].notna())]
# # Now split based on valid_codes
# # # # Standardize both sides for safe comparison
# df['Account Code'] = df['Account Code'].astype(str).str.strip().str.replace('.0', '', regex=False)
# valid_codes = [str(code).strip().replace('.0', '') for code in valid_codes]
...
# # with open(r"C:\Users\gyana\OneDrive\Desktop\MMC CONVERT\Myob To Xero Python\Data Sets\Infracorr Consulting Pty Ltd (Neharika)\XERO DATA\test_write.txt", "w") as f:
# #     f.write("test")

# # Output the dataframes to CSV
# df.to_csv(r"C:\Users\gyana\OneDrive\Desktop\MMC CONVERT\Myob To Xero Python\Data Sets\Infracorr Consulting Pty Ltd (Neharika)\XERO DATA\XERO_SPEND_MONEY.csv", index=False)
# df_bank.to_csv(r"C:\Users\gyana\OneDrive\Desktop\MMC CONVERT\Myob To Xero Python\Data Sets\Infracorr Consulting Pty Ltd (Neharika)\XERO DATA\XERO_BANK_TRANSFER_SPEND_MONEY.csv", index=False)
...
df['Account Code'] = df['Account Code'].astype(str).str.strip().str.replace('.0', '', regex=False)
df_coa["Account Number"] = df_coa["Account Number"].astype(str).str.strip().str.replace('.0', '', regex=False)

# --- Match account codes ---
account_codes = df['Account Code'].dropna().unique()
valid_codes = []
for code in account_codes:
    match = df_coa[df_coa['Account Number'] == code]
    if not match.empty:
...
def map_tax_code(row):
    tax_code = row["Tax"]
    account_code_str = str(row["Account Code"]).strip()
    coa_row = df_coa[df_coa["Account Number"] == account_code_str]
    if not coa_row.empty:
        account_type = coa_row["Account Type"].values[0]
        if tax_code == "GST":
            return tax_code_mapping.get("GST_Income") if account_type in ['Income', 'Other Income'] else tax_code_mapping.get("GST_Expense")
...
df_bank_amount_cleaner = lambda s: s.astype(str).str.replace(",", "", regex=False).str.replace(r"\((.*?)\)", r"-\1", regex=True).replace('', np.nan).astype(float)
df["Amount"] = df_bank_amount_cleaner(df["Amount"])
df["Tax Amount"] = df_bank_amount_cleaner(df["Tax Amount"])

# --- Split df_bank ---
df["Bank"] = df["Bank"].astype(str).str.strip().replace('', np.nan).ffill()
df = df.drop_duplicates()
df = df[df["Date"].astype(str).str.strip() != ""]

# Filter bank-related rows
df_bank = df[df['Account Code'].isin(valid_codes)].reset_index(drop=True)
df = df[~df['Account Code'].isin(valid_codes)].reset_index(drop=True)
...
df_bank.rename(columns=column_mapping_bank, inplace=True)
df_bank["Amount"] = df_bank_amount_cleaner(df_bank["Amount"])
...
print("✅ Success: Files written correctly.")
